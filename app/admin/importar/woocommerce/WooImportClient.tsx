...
/**
 * Client component para el flujo de importación WooCommerce CSV.
 *
 * Estados: idle → previewing → preview-ready → uploading → polling → done/failed
 *
 *  - Drag & drop / file input (acepta hasta 20 MB; el export real pesa ~3.3 MB).
 *  - POST /api/import/woocommerce/preview para enseñar 10 padres antes de
 *    confirmar.
 *  - POST /api/import/woocommerce para crear el job → devuelve jobId.
 *  - Polling cada 2s a /api/import/jobs/[id] hasta DONE/FAILED.
 *  - Renderiza breadcrumbs INFO + errores en directo.
 */
...
export function WooImportClient() {
  ...
  const handleFileSelected = async (f: File) => {
    ...
    fd.append("file", f);
    try {
      const res = await fetch("/api/import/woocommerce/preview", {
        method: "POST",
        body: fd,
      });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Subir export CSV de WooCommerce</CardTitle>
        <CardDescription>
          Arrastra el archivo aquí o haz clic para seleccionarlo. Máximo 20MB.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <>
                ...
                <div className="flex items-center justify-between border-b border-zs-border px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-semibold text-zs-ink">
                      Vista previa (primeros 10 productos detectados)
                    </h4>
                  </div>
                  <Badge variant="outline">{preview.length} productos</Badge>
                </div>
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-zs-surface text-zs-muted">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-right font-medium">Imgs</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zs-border">
                      {preview.map((r) => (
                        <tr key={r.wooId} className="hover:bg-zs-surface/60">
                          ...
                          <td className="px-3 py-2 text-right tabular-nums">
                            {r.retailPrice ?? "—"}
                            {r.salePrice ? (
                              <span className="ml-1 text-[10px] text-zs-red-600">
                                ({r.salePrice})
                              </span>
                            ) : null}
                          </td>
                          <td className="px-3 py-2 text-[11px] uppercase">
                            {r.isSimple ? "simple" : "variable"}
                          </td>
                          <td className="px-3 py-2 text-right tabular-nums">
                            {r.imagesCount}
                          </td>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select value={field.value} onValueChange={field.onChange}>
                        ...
                        <SelectContent>
                          <SelectItem value="create_update">
                            Crear + actualizar
                          </SelectItem>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-zs-muted">
                    Los productos cuya imagen principal se descarga OK pasan
                    automáticamente a <code>ACTIVE</code>.
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4 rounded-xl border border-zs-border bg-white p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold tabular-nums text-zs-blue-900">
                  {job.processedRows} / {job.totalRows || "?"} productos
                </span>
              </div>
