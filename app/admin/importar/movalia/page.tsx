...
import { MovaliaImportClient } from "./MovaliaImportClient";
import { formatDateTimeES } from "@/lib/utils";

export const metadata: Metadata = { title: "Importar de Movalia" };
export const dynamic = "force-dynamic";

export default async function MovaliaImportPage() {
  const enabled =
    process.env.MOVALIA_ENABLED === "true" && !!process.env.MOVALIA_FEED_URL;

  const recent = await db.importJob
    .findMany({
      where: { source: "MOVALIA" },
  ...
  return (
    <div>
      <AdminPageHeader
        title="Importar desde Movalia"
        description="Sincroniza el catálogo Movalia (CSV/JSON/XML)."
        breadcrumbs={[
      ...
      {!enabled && (
        <div className="mb-6 rounded-lg border border-yellow-300 bg-yellow-50 p-4 text-sm text-yellow-900">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold">Conector Movalia no activado</p>
              <p className="mt-1">
                Configura en Vercel:
              </p>
              <ul className="mt-2 list-disc pl-5 text-xs">
                <li>
                  <code>MOVALIA_ENABLED=true</code>
                </li>
                <li>
                  <code>MOVALIA_FEED_URL</code> — URL HTTPS o ruta al fichero
                </li>
                <li>
                  <code>MOVALIA_FEED_FORMAT</code> — <code>csv</code> | <code>json</code> | <code>xml</code>
                </li>
              </ul>
      ...
      <MovaliaImportClient enabled={enabled} />
      ...
      <section className="mt-8">
        ...
        {recent.length === 0 ? (
          <p className="text-sm text-zs-muted">Aún no hay importaciones de Movalia.</p>
        ) : (
