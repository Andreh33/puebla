# Pipeline de imágenes — Zona Sport
...
existe fallback a disco local — todas las imágenes viven siempre en Vercel
...
de 10×10 px guardado en `ProductImage.blurDataUrl` para usar como
`placeholder="blur"` en `next/image`.

Todas las imágenes se sirven con `Cache-Control: public, max-age=31536000`
(1 año) — son inmutables (URL incluye UUID).
...
- `<UploadDropzone />` — drag & drop con compresión cliente
  (browser-image-compression) + barra de progreso por archivo. Llama a
  `/api/upload`.
- `<ImagePicker />` — modal con 2 pestañas (Subir / Galería existente).
  Para campos tipo "imagen única" (cover blog, logo marca…).
- `<ImageSortable />` — wrapper de @dnd-kit/sortable para reordenar las
...
- **Allowlist** de dominios para `/api/upload-from-url`:
  ...
  - `www.johnsmith-sport.com`, `www.mas8000.com`, `shop.movalia.com`
...
- **Filename del cliente** se sanitiza para logging pero **nunca** se usa
  como path en Blob — el path siempre incluye UUID v4.
...
3. El borrado masivo desde la UI **bloquea** URLs que sigan referenciadas
   en DB (devuelve 409) — hay que desasociar primero.

El cron **no borra automáticamente** — sólo lista. Es una decisión
...
Cobertura unitaria:
- `tests/unit/blob-process.test.ts` — pipeline sharp + LQIP.
- `tests/unit/blob-garbage-collect.test.ts` — cruce DB ↔ Blob list.
