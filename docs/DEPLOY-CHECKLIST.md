...
2. Pulsa **"Create Database"** → elige **Neon (Serverless Postgres)** →
   región **fra1** (Frankfurt, mejor latencia desde España) → plan **Free**.
...
2. **"Create Database"** → **Blob** → Connect to Project zonasport.
...
Tarda 10–25 minutos por la descarga de imágenes.
...
- `regions`: `fra1`.
- `crons`: 4 cron jobs (refresh-amazon, refresh-movalia, blob-gc,
...
- `RESEND_API_KEY` (opcional para emails — `re_xxxxxxx`)
...
Vercel → Settings → Domains → añade `zonasport.es`. Te da los registros DNS.
...
```
https://zonasport.es                  → Home con scroll 3D + catálogo
https://zonasport.es/admin/login      → Login admin (SEED_OWNER_EMAIL/PASSWORD)
https://zonasport.es/api/health       → { status: "ok" }
https://zonasport.es/sitemap.xml      → sitemap con TODAS las URLs
https://zonasport.es/manifest.webmanifest → PWA manifest
```
